...
const Maps = ({ small = false, setCoordinates }) => {
    ...
    const [lescoords, setCoords] = useState([])
    ...
    const loadData = async () => {
        try {
            // const res = await fetch("https:tri9i.herokuapp.com/allSchools",{
            //     method:"get",
            //     headers: {
            ...
            // console.log(data)
            const res = await axios.get('/allSchools', {
                headers: {
                    'Content-Type': 'application/json'
                }
            })
